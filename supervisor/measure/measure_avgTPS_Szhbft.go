...
type TestModule_avgTPS_Szhbft struct {
	epochID      int
...
func NewTestModule_avgTPS_Szhbft() *TestModule_avgTPS_Szhbft {
	return &TestModule_avgTPS_Szhbft{
		epochID:      -1,
...
func (tat *TestModule_avgTPS_Szhbft) OutputMetricName() string {
	return "Average_TPS"
}

// add the number of excuted txs, and change the time records
func (tat *TestModule_avgTPS_Szhbft) UpdateMeasureRecord(b *message.BlockInfoMsg) {
	...
	for tat.epochID < epochid {
		...
		tat.szhbft1TxNum = append(tat.szhbft1TxNum, 0)
		tat.szhbft2TxNum = append(tat.szhbft2TxNum, 0)
	...
	tat.szhbft1TxNum[epochid] += r1TxNum
	tat.szhbft2TxNum[epochid] += r2TxNum
...
func (tat *TestModule_avgTPS_Szhbft) HandleExtraMessage([]byte) {}

// output the average TPS
func (tat *TestModule_avgTPS_Szhbft) OutputRecord() (perEpochTPS []float64, totalTPS float64) {
	tat.writeToCSV()
...
func (tat *TestModule_avgTPS_Szhbft) writeToCSV() {
	...
	for eid, exTxNum := range tat.excutedTxNum {
		...
		csvLine := []string{
			...
			strconv.Itoa(tat.szhbft1TxNum[eid]),
			strconv.Itoa(tat.szhbft2TxNum[eid]),
