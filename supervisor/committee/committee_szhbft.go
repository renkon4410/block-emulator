...
type SzhbftCommitteeModule struct {
	csvPath      string
	dataTotalNum int
	nowDataNum   int
	batchDataNum int
...
func NewSzhbftCommitteeModule(Ip_nodeTable map[uint64]map[uint64]string, Ss *signal.StopSignal, slog *supervisor_log.SupervisorLog, csvFilePath string, dataNum, batchNum int) *SzhbftCommitteeModule {
	return &SzhbftCommitteeModule{
		csvPath:      csvFilePath,
...
func (rthm *SzhbftCommitteeModule) HandleOtherMessage([]byte) {}

func (rthm *SzhbftCommitteeModule) txSending(txlist []*core.Transaction) {
...
func (rthm *SzhbftCommitteeModule) MsgSendingControl() {
...
// no operation here
func (rthm *SzhbftCommitteeModule) HandleBlockInfo(b *message.BlockInfoMsg) {
